...
public class ArcadeDriveCmd extends a_cmd {
	...
	public void autoDriveTwist(double driveSpeedTwist, double driveTime){
		...
		driveSpeedTwist = driveStick.pX; //will be controlling Twist function based on the way the Arcade Drive is coded
		}
		else{
			driveStick.pY = 0.0;
		}
	...
	public void driveWhileLessThanRF(double limit){
		
		
		if (averageDistance(currentDistance) < limit){
	...
	public void driveWhileGreaterThanRF(double limit){
		if (averageDistance(currentDistance) > limit){
			frontLeftMotor.set(0);
	    	frontRightMotor.set(0);
	    	rearLeftMotor.set(0);
	...
	public double averageDistance(double currentDistance){
